...
void servo_arm_down(){
	...
	Sleep(50); // make sure that the motor has started before entering the loop
	while(servo_arm_is_running() && count<50){ // waiting until the speed of the motor has reached 0.
		Sleep(20);
		count++;
	}
...
void absolute_servo_sonar(int angle){
	...
	Sleep(50); // make sure that the motor has started before entering the loop
	while(servo_sonar_is_running() && count<50){ // waiting until the speed of the motor has reached 0.
		Sleep(20);
		count++;
	}
